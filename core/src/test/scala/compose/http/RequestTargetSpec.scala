...
class RequestTargetSpec extends compose.Spec {
  ...
  "The request target parser" should {
    ...
    "recognize absolute targets" in {
      RequestTarget.parse("https://absolute.example.com/path?query#fragment") shouldBe a[RequestTarget.Absolute]
    }
  ...
  "Absolute targets" should {
    "have a string representation" in {
      val stringRepr = RequestTarget.Absolute(new URI("https://absolute.example.com/path?query#fragment")).toString
  ...
  "Origin targets" which {
    "are well-formed" should {
      "have string representations" in {
        val stringRepr = RequestTarget.Path("/some/path", Map("a" -> Seq("a1", "a2"), "b" -> Seq("b1"))).toString
