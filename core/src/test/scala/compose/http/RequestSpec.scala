...
class RequestSpec extends compose.Spec {
  "The request parser" should {
    "parse an HTTP request" in {
      ...
      parsedRequest.value.headers.getAll("User-Agent") should contain theSameElementsAs Seq(
        "scalatest"
      )
      parsedRequest.value.headers.getAll("Accept") should contain theSameElementsAs Seq("*/*")
      parsedRequest.value.headers.getAll("Content-Type") should contain theSameElementsAs Seq(
        "text/plain"
      )
